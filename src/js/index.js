...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const arrowDowm = document.querySelector(".shopping__btn-box > img");

  // Перевірка наявності елементів на сторінці
  if (btn && content && arrowDowm) {
    btn.addEventListener("click", function () {
      toggleDropdown();
    });
  ...
  function toggleDropdown() {
    ...
    arrowDowm.classList.toggle("arrow-rotate");
  }
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const countries = ["Afghanistan",
	...
	"Zambia",
	"Zimbabwe",
	"Åland Islands"]; // список країн

  // Функція для оновлення списку країн
  function updateCountryList(filteredCountries) {
    countryList.innerHTML = ""; // очистити список перед оновленням
    filteredCountries.forEach(function (country) {
      const listItem = document.createElement("li");
      listItem.textContent = country;
      listItem.addEventListener("click", function () {
        countryInput.value = country;
        countryList.classList.remove("country-list-active"); // при виборі країни список зникає
      });
  ...
  countryInput.addEventListener("input", function () {
    const inputText = countryInput.value.toLowerCase(); // отримати введений текст і перевести його в нижній регістр
    const filteredCountries = countries.filter(function (country) {
      return country.toLowerCase().startsWith(inputText); // фільтрувати країни, які починаються з введеного тексту
    });
    updateCountryList(filteredCountries); // оновити список країн
    countryList.classList.add("country-list-active"); // показати список
  });

  // Обробник кліку на документ, щоб закрити список при кліку за межами
  document.addEventListener("click", function (event) {
    if (!countryList.contains(event.target)) {
      countryList.classList.remove("country-list-active"); // при кліку за межами список приховується
    }
  });
});
*/
